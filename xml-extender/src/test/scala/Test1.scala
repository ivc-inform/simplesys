...
object Test extends Assert {
  ...
  private def test1 {
    ...
    assertSame(x0.child, Nil)
    assertSame(x00.child, Nil)
    ...
    for (z <- xg.child) {
      println(z.toString() + {
        if (z.isInstanceOf[Text]) "(is text node ' ')" else ""
      })
    }

    val xh = <hello>
      {for (x <- List(1, 2, 3, 4) if x % 2 == 0) yield x}
    </hello>
  ...
  private def test3 {
    ...
    object parser extends xml.parsing.ConstructingParser(s, false /*ignore ws*/) {
      override def replacementText(entityName: String): io.Source = {
        entityName match {
          case "nbsp" => io.Source.fromString("\u003C");
          case _ => super.replacementText(entityName);
        }
      }
      nextch; // !!important, to initialize the parser
    }
    val parsed = parser.document().docElem // parse the source as element
    // alternatively, we could call document()
    //println(0x0022.toChar.toString)
    println(parsed)
  }
