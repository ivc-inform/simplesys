...
object MultiSortDialogDyn {
  implicit def MultiSortDialog2JsonObject(in: MultiSortDialogDyn): JsonObject = in json
}

class MultiSortDialogDyn(override val useSelfName: Boolean = false) extends WindowDyn with MultiSortDialogDynInit {
  override val selfName = "MultiSortDialog"
  ...
  private[this] var _applyButton: AutoChild = applyButton
  def ApplyButton = _applyButton
  def ApplyButton_=(value: AutoChild) {
    if (_applyButton == value) return
    json("applyButton") = value
    _applyButton = value
  }

  private[this] var _applyButtonTitle: String = applyButtonTitle
  def ApplyButtonTitle = _applyButtonTitle
  def ApplyButtonTitle_=(value: String) {
    if (_applyButtonTitle == value) return
    json("applyButtonTitle") = value
