import com.simplesys.sourcegenJS.SourceGenPlugin.{classNameToRun, generatorJSSettings, outputJSDir, withBeautifier, _}
...
//lazy val boneCPWrapper = Project(id = "bonecp-wrapper", base = file("bonecp-wrapper")).dependsOn(common, xmlExtender, configWrapper, doobieExtender).settings(
...
lazy val iscComponents = Project(id = "isc-components", base = file("isc-components")).dependsOn(scalaIOExtender, iscWrapper).enablePlugins(TranspileCoffeeScript).
  settings(generatorJSSettings).
  settings(
      libraryDependencies ++= Seq(
          CommonDeps.servletAPI.value % "provided",
          CommonDeps.scalaTest.value % Test
