...
object TreeGridEditorDyn {

    implicit def GridEditorDyn2JsonObject(in: ListGridEditorDyn): JsonObject = in json
}

class TreeGridEditorDyn(override val useSelfName: Boolean = false) extends VLayoutDyn with TreeGridEditorDynInit {
    override val selfName = "TreeGridEditor"
    ...
    def CanAcceptDroppedRecords = json.getJsonElement("canAcceptDroppedRecords1").getOrElse(canAcceptDroppedRecords)
    def CanAcceptDroppedRecords_=(value: JsonElement) {
        if (CanAcceptDroppedRecords == value) return
        json("canAcceptDroppedRecords") = value
    }

    def CanReparentNodes = json.getJsonElement("canReparentNodes1").getOrElse(canReparentNodes)
    def CanReparentNodes_=(value: JsonElement) {
        if (CanReparentNodes == value) return
        json("canReparentNodes") = value
    }

    def FolderDrop = json.getJsonElement("folderDrop1").getOrElse(folderDrop)
    def FolderDrop_=(value: JsonElement) {
        if (FolderDrop == value) return
        json("folderDrop") = value
    }

    def FolderDropImage = json.getSCImgURL("folderDropImage").getOrElse(folderDropImage)
    def FolderDropImage_=(value: SCImgURL) {
        if (FolderDropImage == value) return
        json("folderDropImage") = value
    }

    def FolderIcon = json.getSCImgURL("folderIcon").getOrElse(folderIcon)
    def FolderIcon_=(value: SCImgURL) {
        if (FolderIcon == value) return
        json("folderIcon") = value
    }

    def NodeIcon = json.getSCImgURL("nodeIcon").getOrElse(nodeIcon)
    def NodeIcon_=(value: SCImgURL) {
        if (NodeIcon == value) return
        json("nodeIcon") = value
    }

    def ShowOpenIcons = json.getJsonElement("showOpenIcons1").getOrElse(showOpenIcons)
    def ShowOpenIcons_=(value: JsonElement) {
        if (ShowOpenIcons == value) return
        json("showOpenIcons") = value
    }

    def LoadDataOnDemand = json.getJsonElement("loadDataOnDemand1").getOrElse(loadDataOnDemand)
    def LoadDataOnDemand_=(value: JsonElement) {
        if (LoadDataOnDemand == value) return
