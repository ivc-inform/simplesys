...
object Message {
    ...
    def apply(channels: String*) = new Message(id = "", data = fromJsonObject(JsonObject.empty), channels = channels.toSet, event = None)
...
object MessageExt {
    ...
    def apply(channels: SseChannel*) = new Message(id = "", data = fromJsonObject(JsonObject.empty), channels = channels.map(_.serrialize).toSet, event = None)
