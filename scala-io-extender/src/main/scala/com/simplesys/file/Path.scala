...
import Path.fail
import com.simplesys.io.Output
import com.simplesys.io.StandardOpenOption
import com.simplesys.io.Input
...
object Path {
    ...
    /**
      ...
      *
      * For more on matching Paths see [[com.simplesys.file.PathMatcher]]
      */
...
abstract class Path(val fileSystem: FileSystem) extends FileOps with PathFinder[Path] with Ordered[Path] {
    ...
    /**
      ...
      */
    def simpleName: String = name dropRight extension.map {
        1 + _.size
    }.getOrElse(0)
    /**
    ...
    /**
      ...
      */
    def nonExistent = try !exists catch {
    ...
    def access_=(accessModes: String): Unit = {
        ...
        val (modifier, modes) = if (accessModes.headOption forall {
            "+-" contains _
        }) {
    ...
    def copyTo[P <: Path](target: P,
        ...
        if (isDirectory) {
            ...
            if (createParents) {
                target.parent.foreach {
                    _.createDirectory(true, false)
                }
            }
            copyDataTo(target)
        }
