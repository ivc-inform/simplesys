 ...
 * parameter -Djava.protocol.handler.pkgs=com.simplesys.file
...
object PathURLStreamHandlerFactory extends URLStreamHandlerFactory {
  ...
  def createURLStreamHandler(protocol:String) = {
    if(supported contains protocol) Class.forName("com.simplesys.file."+protocol+".Handler").newInstance.asInstanceOf[URLStreamHandler]
    else null
  }
}
