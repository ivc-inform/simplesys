...
import com.simplesys.file.PathMatcher.FunctionMatcher
...
object PathMatcher {
   ...
   *          on the path object.
   */
  final class AttributeMatcher (attributes: com.simplesys.file.attributes.FileAttribute[_]*) extends PathMatcher[Path] {
    def apply(path: Path) = {
  ...
  object AttributeMatcher {
      def apply(attributes: com.simplesys.file.attributes.FileAttribute[_]*) = new AttributeMatcher(attributes:_*)
  }
