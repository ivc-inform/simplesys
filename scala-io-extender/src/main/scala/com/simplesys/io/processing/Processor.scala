 ...
 * options beyond what is available using the normal collections-like API available in [[com.simplesys.io.LongTraversable]].
 *
 * {{{
 * val firstElems = for {
 ...
 *
 * For more details on how the processing API is used look at the [[com.simplesys.io.processing.ProcessorAPI]] documentation.
 *
 * It is possible for a Processor to be empty so acquireAndGet returns an Option to support this case.  Consider:
 *
 * {{{
 * import com.simplesys.io.JavaConverters._
 * val result = for {
 ...
 *
 * @see [[com.simplesys.io.processing.ProcessorAPI]]
 *
...
trait Processor[+A] {
   ...
   * (See [[com.simplesys.io.LongTraversable]].async)
   *
   ...
   */
  def future = {
    implicit val executionContext = com.simplesys.io.executionContext
   ...
   * like a Processor created by an [[com.simplesys.io.processing.OpenOutput]] or [[com.simplesys.io.processing.OpenSeekable]]
...
private[processing] class WithFilter[+A](base:Processor[A], filter: A=>Boolean) extends Processor[A] {
    ...
    override def foreach[U](f: A => U): Unit = base.init.execute.filter(filter).foreach(f)
    override def map[U](f: A => U) = processFactory(base.init.execute.filter(filter).map(f))
    override def flatMap[U](f: A => Processor[U]) = processFactory(base.init.execute.filter(filter).flatMap(f(_).init.execute))
}

private[processing] class TimingOutProcessor[+A] (base: Processor[A], timeout: Duration) extends Processor[A] {
  private implicit val executionContext = com.simplesys.io.executionContext
  def context = base.context
  private[processing] def init = {
