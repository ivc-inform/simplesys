...
import resource._
import com.simplesys.io.CloseAction._
import com.simplesys.io.StandardOpenOption._
import com.simplesys.io.managed._
import com.simplesys.io.nio.SeekableFileChannel
import com.simplesys.io.support.FileUtils
...
/**
  ...
  *
  * A common question about Resource is: "Why no simply expose [[com.simplesys.io.Input]],
  * [[com.simplesys.io.Output]], [[com.simplesys.io.Seekable]], [[com.simplesys.io.ReadChars]], [[com.simplesys.io.WriteChars]]?
  ...
  *
  * Perhaps they need to interact with a Java API which does not use the [[com.simplesys.io.Input]], etc...
    ...
    final def acquireFor[B](f: R => B): ExtractedEither[List[Throwable], B] = {
        ...
        resourceEither match {
            case Left(t) =>
                ...
                ExtractedEither(handlerValue getOrElse Left(List(t)))
            case Right(resource) =>
                val result =
                ...
                ExtractedEither(if (handleError) {
                    try {
...
/**
  * An Resource object that is a also an [[com.simplesys.io.Input]].  This trait adds methods
  * for converting between common io types such as to a [[com.simplesys.io.managed.ReaderResource]] or
  * [[com.simplesys.io.managed.ReadableByteChannelResource]]
  *
  * @tparam R
...
trait InputResource[+R] extends Resource[R] with Input with ResourceOps[R, InputResource[R]] {
    ...
    /**
      * Obtain the [[com.simplesys.io.managed.InputStreamResource]](typically) version of this object.
      *
      * @return the [[com.simplesys.io.managed.InputStreamResource]](typically) version of this object.
      */
    ...
    /**
      * Obtain the [[com.simplesys.io.managed.ReadableByteChannelResource]](typically) version of this object.
      *
      * @return the [[com.simplesys.io.managed.ReadableByteChannelResource]](typically) version of this object.
      */
...
/**
  * An object that in addition to being a resource is also a [[com.simplesys.io.ReadChars]] Resource.
  *
...
/**
  * An Resource object that is a also an [[com.simplesys.io.Output]].  This trait adds methods
  * for converting between common io types such as to a [[com.simplesys.io.managed.WriterResource]] or
  * [[com.simplesys.io.managed.WritableByteChannelResource]]
  *
  * @tparam R
  * the type of underlying resource that is created
  * @author Jesse Eichar
...
trait OutputResource[+R] extends Resource[R] with Output with ResourceOps[R, OutputResource[R]] {
    /**
      * Obtain the [[com.simplesys.io.OutputResource]](typically) version of this object.
      *
      * @return the [[com.simplesys.io.OutputResource]](typically) version of this object.
      */
    def outputStream: OutputResource[OutputStream]
    /**
      * Obtain the [[com.simplesys.io.WriteCharsResource]] version of this object.
      *
      * @param sourceCodec the codec used to convert from bytes to characters
      * @return the [[com.simplesys.io.WriteCharsResource]] version of this object.
      */
    def writer(implicit sourceCodec: Codec = Codec.default): WriteCharsResource[Writer]
    /**
      * Obtain the [[com.simplesys.io.managed.WritableByteChannelResource]](typically) version of this object.
      *
      * @return the [[com.simplesys.io.managed.WritableByteChannelResource]](typically) version of this object.
      */
...
/**
  * An object that in addition to being a resource is also a [[com.simplesys.io.WriteChars]] Resource.
  *
  * @tparam R The underlying resource
  * @author Jesse Eichar
  * @since 1.0
...
object Resource {
    ...
    /**
      * Create an [[com.simplesys.io.managed.InputStreamResource]] from an InputStream or subclass
      *
      * $openDisclaimer
      *
    ...
    def fromRandomAccessFile(opener: => RandomAccessFile): SeekableByteChannelResource[SeekableFileChannel] = {
        ...
        def sizeFunc = () => allCatch.opt {
            resource.managed(opener).acquireAndGet {
                _.length match {
                    case len if len > -1 => len
                }
            }
        }
        new SeekableByteChannelResource[SeekableFileChannel](open, new ResourceContext {
            override def descName = PrefixedName("RandomAccessFile")
        }, Noop, sizeFunc, Some(ReadWrite))
    }
    ...
    def fromURL(url: URL): InputStreamResource[InputStream] = {
        ...
        new InputStreamResource(url.openStream, new ResourceContext {
            override def descName = KnownName(url.toExternalForm)
        }, Noop, sizeFunc)
    }
    ...
    def fromFile(file: File): SeekableByteChannelResource[SeekableByteChannel] = {
        ...
        new SeekableByteChannelResource[SeekableFileChannel](open, new ResourceContext {
            override def descName = KnownName(file.getPath)
        }, Noop, sizeFunc, None)
    }
    /**
