...
object Scr_ScenarioBoExeCodeError {
     def apply(implicit dataSource: BoneCPDataSource) = new Scr_ScenarioBoExeCodeError(alias = SQLAlias(strEmpty))    
     def apply(alias: SQLAlias)(implicit dataSource: BoneCPDataSource) = new Scr_ScenarioBoExeCodeError(alias = alias)    
...
class Scr_ScenarioBoExeCodeError(val alias: SQLAlias)(implicit val dataSource: BoneCPDataSource) extends ClassBO[Scr_ScenarioBoExeCodeError]  {
///////////////////////////////////////// Class: Scr_Scenario, group: scenarios /////////////////////////////////////////    
    
     def this()(implicit dataSource: BoneCPDataSource) = this(SQLAlias(strEmpty))(dataSource)    
    
     def init[FT <: Product with FieldProduct](columns: FT = null, join: JoinParam = null, where: WhereParam = null, orderBy: OrderByParam = null): Scr_ScenarioBoExeCodeError = {
        _columns = columns
        _join Join join
        _where Where where
...
///////////////////////////////////////////////// Tables /////////////////////////////////////////////////    
    ...
    val Scr_ScenarioTbl = new Scr_ScenarioTblExeCodeError(alias = alias + "T0".als)    
    override val fromTable = Scr_ScenarioTbl    
//////////////////////////////////////////////// End Tables ////////////////////////////////////////////////    
////////////////////////////////////////////// Columns for select //////////////////////////////////////////////    
    val exeCodeError = ClobOptionColumn(name = "CLBEXECODEERROR", nameInBo = "exeCodeError", caption = "Execute Code Error", tableColumn = Scr_ScenarioTbl.exeCodeErrorOption)    
    ...
    // For select tuple: (exeCodeError: Array[Clob], id_scenario: Long)    
    
    type ColumnTypes =     TupleSS2[Array[Clob], Long]    
    val allColumns = exeCodeError ~ id_scenario    
    val allColumns1 = Seq(exeCodeError , id_scenario)    
    val constraints = 
...
////////////////////////////////////////////// Table: Scr_Scenario //////////////////////////////////////////////    
    val exeCodeErrorScr_ScenarioTbl = Scr_ScenarioTbl.exeCodeErrorOption    
...
/////////////////////////////////////////////// Fetch Product ///////////////////////////////////////////////    
     ...
     def selectPIterator[FT <: Product with FieldProduct](columns: FT = allColumns, join: JoinParam = null, where: WhereParam = null, orderBy: OrderByParam = null, fetchSize: Int = dataSource.Config.FetchSize, dsRequest: DSRequest = null): ValidationExIterator[Iterator[Scr_ScenarioExeCodeError]] = 
    ...
    case TupleSS2(exeCodeError: Array[Clob], id_scenario: Long) => 
        Scr_ScenarioExeCodeError(exeCodeError = exeCodeError, id_scenario = id_scenario)
}    
    
     def selectPList(join: JoinParam = null, where: WhereParam = null, orderBy: OrderByParam = null, fetchSize: Int = dataSource.Config.FetchSize, dsRequest: DSRequest = null): ValidationEx[List[Scr_ScenarioExeCodeError]] = 
     ...
     def selectPOne(join: JoinParam = null, where: WhereParam = null): ValidationEx[Scr_ScenarioExeCodeError] = 
                selectOneRoot(columns = allColumns, from = From(Scr_ScenarioTbl), join = join, where = where, discriminator = null).result match {
            case Success(TupleSS2(exeCodeError: Array[Clob], id_scenario: Long)) => 
                ValidationEx(Success(Scr_ScenarioExeCodeError(exeCodeError = exeCodeError, id_scenario = id_scenario)))
            case Failure(x) => 
                ValidationEx(Failure(x))
            case x => 
                throw new RuntimeException(s"Bad branch. (${x})")
...
///////////////////////////////////////////////// Insert /////////////////////////////////////////////////    
     ...
     def insert(values: TupleSS2[Array[Clob], Long]*): ValidationEx[List[Int]] = 
        transaction(dataSource)
            {
            connection =>
                Scr_ScenarioTbl.insertWithoutCommit(connection = connection, columns = exeCodeErrorScr_ScenarioTbl ~ id_scenarioScr_ScenarioTbl, 
                    values map
                        {
                        value =>
                            Insert(exeCodeErrorScr_ScenarioTbl, value _1) Insert(id_scenarioScr_ScenarioTbl, value _2)
                    })
        }    
    
     def insertP(values: Scr_ScenarioExeCodeError*): ValidationEx[List[Int]] = 
        transaction(dataSource)
            {
            connection =>
                Scr_ScenarioTbl.insertWithoutCommit(connection = connection, columns = exeCodeErrorScr_ScenarioTbl ~ id_scenarioScr_ScenarioTbl, 
                    values map
                        {
                        value =>
                            Insert(exeCodeErrorScr_ScenarioTbl, value.exeCodeError) Insert(id_scenarioScr_ScenarioTbl, value.id_scenario)
                    })
        }    
     ...
     def insertPWithoutCommit(connection: Connection, values: Scr_ScenarioExeCodeError*): List[Int] = {
        Scr_ScenarioTbl.insertWithoutCommit(connection = connection, columns = exeCodeErrorScr_ScenarioTbl ~ id_scenarioScr_ScenarioTbl, 
            values map
                {
                value =>
...
///////////////////////////////////////////////// Update /////////////////////////////////////////////////    
     def update(value: TupleSS2[Array[Clob], Long], where: WhereParam): ValidationEx[List[Int]] = 
        transaction(dataSource)
            {
            connection =>
                updateWithoutCommit(connection = connection, setters = Set(exeCodeError, value _1) Set(id_scenario, value _2), where = where, table = SQLTable(Scr_ScenarioTbl.databaseTablename))
        }    
    
     def updateP(value: Scr_ScenarioExeCodeError, where: WhereParam): ValidationEx[List[Int]] = 
        transaction(dataSource)
            {
            connection =>
                updateWithoutCommit(connection = connection, setters = Set(exeCodeError, value.exeCodeError) Set(id_scenario, value.id_scenario), where = where, table = SQLTable(Scr_ScenarioTbl.databaseTablename))
        }    
     ...
     def updatePWithoutCommit(connection: Connection, value: Scr_ScenarioExeCodeError, where: WhereParam): List[Int] = {
        updateWithoutCommit(connection = connection, setters = Set(exeCodeError, value.exeCodeError) Set(id_scenario, value.id_scenario), where = where, table = SQLTable(Scr_ScenarioTbl.databaseTablename))
    }    
//////////////////////////////////////////////// End Update ////////////////////////////////////////////////    
