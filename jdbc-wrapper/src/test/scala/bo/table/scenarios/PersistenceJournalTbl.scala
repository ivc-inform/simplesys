...
import scenarios.PersistenceJournal
import com.simplesys.jdbc.control.clob._
import com.simplesys.common.array._
import com.simplesys.tuple._

 
object PersistenceJournalTbl {
     def apply(implicit dataSource: BoneCPDataSource) = new PersistenceJournalTbl(alias = SQLAlias(strEmpty))    
     def apply(alias: SQLAlias)(implicit dataSource: BoneCPDataSource) = new PersistenceJournalTbl(alias = alias)    
    
    val objectName = "PersistenceJournal"    
    val groupName = "scenarios"
}
 
class PersistenceJournalTbl(val alias: SQLAlias)(implicit val dataSource: BoneCPDataSource) extends Table[PersistenceJournalTbl]  {
...
//////////////////////////////////////////////// P Methods ////////////////////////////////////////////////    
     ...
     def insertP(values: PersistenceJournal*): ValidationEx[List[Int]] = insertRoot(allColumns, 
    values map
        {
        value =>
     ...
     def batch4PInsert(preparedStatement: PreparedStatement, values: PersistenceJournal*): Unit = batch4Insert(preparedStatement, 
    values map
        {
        value =>
            Insert(created, value.created) Insert(deletedOption, value.deleted) Insert(orderingOption, value.ordering) Insert(persistence_id, value.persistence_id) Insert(sequence_number, value.sequence_number) Insert(tagsOption, value.tags) Insert(timestamp, value.timestamp) 
     ...
     def insertPWithoutCommit(connection: Connection, values: PersistenceJournal*): List[Int] = insertWithoutCommit(connection, allColumns, 
    values map
        {
        value =>
            Insert(created, value.created) Insert(deletedOption, value.deleted) Insert(orderingOption, value.ordering) Insert(persistence_id, value.persistence_id) Insert(sequence_number, value.sequence_number) Insert(tagsOption, value.tags) Insert(timestamp, value.timestamp) 
