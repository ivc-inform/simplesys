...
import scenarios.PersistenceSnapshot_Debug
import com.simplesys.jdbc.control.clob._
import com.simplesys.common.array._
import com.simplesys.tuple._

 
object PersistenceSnapshot_DebugTbl {
     def apply(implicit dataSource: BoneCPDataSource) = new PersistenceSnapshot_DebugTbl(alias = SQLAlias(strEmpty))    
     def apply(alias: SQLAlias)(implicit dataSource: BoneCPDataSource) = new PersistenceSnapshot_DebugTbl(alias = alias)    
    
    val objectName = "PersistenceSnapshot_Debug"    
    val groupName = "scenarios"
}
 
class PersistenceSnapshot_DebugTbl(val alias: SQLAlias)(implicit val dataSource: BoneCPDataSource) extends Table[PersistenceSnapshot_DebugTbl]  {
...
//////////////////////////////////////////////// P Methods ////////////////////////////////////////////////    
     ...
     def insertP(values: PersistenceSnapshot_Debug*): ValidationEx[List[Int]] = insertRoot(allColumns, 
    values map
        {
        value =>
     ...
     def batch4PInsert(preparedStatement: PreparedStatement, values: PersistenceSnapshot_Debug*): Unit = batch4Insert(preparedStatement, 
    values map
        {
        value =>
            Insert(createdOption, value.created) Insert(persistence_id, value.persistence_id) Insert(sequence_number, value.sequence_number) Insert(timestamp, value.timestamp) 
     ...
     def insertPWithoutCommit(connection: Connection, values: PersistenceSnapshot_Debug*): List[Int] = insertWithoutCommit(connection, allColumns, 
    values map
        {
        value =>
            Insert(createdOption, value.created) Insert(persistence_id, value.persistence_id) Insert(sequence_number, value.sequence_number) Insert(timestamp, value.timestamp) 
