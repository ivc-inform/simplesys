...
import com.simplesys.circe.Circe.inputStream2Sting
...
object JDBC extends Logging{
    ...
    implicit def arrayInputStream2JsonElement(value: Array[InputStream]): Option[Json] = if (value.length == 0) None else Some(fromString(inputStream2Sting(value.head)))
